...
export default function BlogPost(){
    ...
    useEffect(()=>{
        const fetchPosts= async ()=>{
                try{
                    const response = await fetch('https://jsonplaceholder.typicode.com/posts?_limit=1000');
    ...
    if(loading){
        ...
        // return(<>{list}</>);
        return (posts.map(post=>(<div key={post.id}><h2>{post.title}</h2><p>{post.body}</p></div>)) );
    }
